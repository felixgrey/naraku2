...
function () {
  function ListenerManager(dhc) {
    ...
    this.devLog = _devMode ? dh.devLog.createLog("ListenerManager=".concat(this._key)) : _Utils.udFun;
    this.errLog = dh.errLog.createLog("ListenerManager=".concat(this._key));
  ...
  _createClass(ListenerManager, [{
      ...
      var checkReady = function checkReady() {
        ...
        try {
          for (var _iterator = names[Symbol.iterator](), _step; !(_iteratorNormalCompletion = (_step = _iterator.next()).done); _iteratorNormalCompletion = true) {
            ...
            if (!_this3._dh.hasData(_name)) {
              return;
            } else {
              dataList.push(_this3._dh.get(_name));
            }
          }
        } catch (err) {
    ...
    value: function whenAll(names, callback) {
      ...
      if (names.filter(function (_name) {
        return _this4._dh.hasData(_name);
      }).length === names.length) {
