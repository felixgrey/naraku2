...
var _Union = require("../Common/Union.js");
...
function clearStatus(name, stopKey, _callName) {
  ...
  (0, _Union.getDevMode)() && devLog("clearStatus: ".concat(_callName), name, stopKey, JSON.stringify(stopKeyMap));
...
function addFetcher(name, url) {
  ...
  (0, _Union.getDevMode)() && devLog('addFetcher: ', name, url, method, extend);
  fetchMap[name] = {
    url,
    method,
    extend
...
function removeFetcher(name) {
  ...
  (0, _Union.getDevMode)() && devLog('removeFetcher: ', name);
...
function initFetcher(callback) {
  ...
  if (fetcher) {
    ...
    (0, _Union.getDevMode)() && devLog('initialize fetch again.');
  }

  (0, _Union.getDevMode)() && devLog('run initFetcher.');
...
function stopFetchData(stopKey) {
  if (!stopKeyMap[stopKey]) {
    (0, _Union.getDevMode)() && devLog("stopKey ".concat(stopKey, " not existed."));
    return;
  }
  ...
  (0, _Union.getDevMode)() && devLog('stopFetchData', name, stopKey);
  callback();
}
...
function fetchData(name) {
  ...
  var extend2 = _objectSpread({
    dataType: 'json',
    beforeSend: _Utils.sameFun,
    afterResponse: _Utils.sameFun
  }, extend);

  var beforeFetch = extend2.beforeFetch || _Utils.sameFun;
  var afterFetch = extend2.afterFetch || _Utils.sameFun;
  ...
  var fetchPromise = new Promise((resolve, reject) => {
    setResult = data => {
      (0, _Union.getDevMode)() && devLog('fetch success and run setResult', data);
    ...
    if (!(0, _Utils.isNvl)(stopKey)) {
      if (stopKeyMap[stopKey]) {
        ...
        stopKeyMap[stopKey] = {
          name,
          callback: () => {
            (0, _Union.getDevMode)() && devLog('stop and reject whthout onStop.');
        ...
        onStop = function onStop() {
          ...
          stopKeyMap[stopKey] = {
            name,
            callback: () => {
              (0, _Union.getDevMode)() && devLog('run onStop callback and reject.');
    ...
    setError = err => {
      (0, _Union.getDevMode)() && devLog('setError and reject.', err);
      reject(err);
    };
  });
  ...
  return fetchPromise.finally(() => {
    (0, _Union.getDevMode)() && devLog('fetch finally.');
