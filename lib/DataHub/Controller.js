...
var publicMethods = ['createController', 'watch', 'isLoading', 'isLocked', 'isWillRefresh', 'getDataHub', 'getController'];
var {
  publicMethod
} = _Container.default;
var Controller = (_class = class Controller extends _Container.default {
  ...
  refresh() {
    ...
    for (var callback of this.watchSet) {
      callback();
    }
  }
  ...
  initWatch() {
    var lagRefresh = () => {
      ...
      var refreshRate = (0, _Union.getRefreshRate)();
      var time = Date.now() - this.refreshTime;
    ...
    var off1 = this.emitter.on('$$data', lagRefresh);
    var off2 = this.emitter.on('$$status', lagRefresh);
    this.emitter.once("$$destroy:Controller:".concat(this.key), () => {
      off1();
      off2();
    });
  }
