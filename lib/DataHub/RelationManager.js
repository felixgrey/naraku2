      ...
      fetcher: (value, cfg) => {
        ...
        var checkReady = function checkReady() {
          ...
          for (var dep of dependence) {
            var depStore = _this.dataHub.getDataStore(dep);

            if (!depStore.hasData()) {
              if (_this.dataStore.hasData()) {
                var _param = {
                  name: _this.name,
                  clear: true,
                  force
                ...
                _this.dataHubController.fetchManager.fetchStoreData(_param);
              }
          ...
          for (var ft of filter) {
            Object.assign(submitData, _this.dataHub.getDataStore(ft).first());
          }
          ...
          var param = {
            ...
            before: () => {
              whenThem.forEach(storeName => {
                _this.dataHub.getDataStore(storeName).lock();
              });
            },
            after: () => {
              whenThem.forEach(storeName => {
                _this.dataHub.getDataStore(storeName).unLock();
              });
            }
          };
          ...
          _this.dataHubController.fetchManager.fetchStoreData(param);
        };
