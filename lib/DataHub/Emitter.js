...
function () {
  ...
  _createClass(Emitter, [{
    ...
    value: function _onAndOnce(name, callback, once) {
      ...
      if (this._destroyed) {
        this.errLog("can't run 'on/once' event='".concat(name, "' after emitter=").concat(this._key, " destroy."));
        return _Utils.udFun;
      }
    ...
    value: function emit(name) {
      ...
      if (this._destroyed) {
        this.errLog("can't run 'emit' event='".concat(name, "' after emitter=").concat(this._key, " destroy."));
        return;
      }
    ...
    value: function destroy() {
      ...
      this.emit('$$destroy:emitter', this._key); // this.devLog(`emitter=${this._key} destroyed.`);
