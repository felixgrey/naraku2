...
function () {
  ...
  _createClass(DataHub, [{
    key: "getPublicFunction",
    value: function getPublicFunction() {
      if (this._destroyed) {
    ...
    value: function setStatus(name, value) {
      ...
      if (value === 'locked') {
        this.errLog("please use \"dataHub.lock\" to lock \"".concat(name, "\"."));
        return;
      }

      if (value === 'error') {
        this.errLog("please use \"dataHub.setError\" to set error to \"".concat(name, "\"."));
        return;
      }
