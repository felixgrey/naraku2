...
var dataOpMethods = ['set', 'get', 'remove', 'hasData', 'setStatus', 'getStatus', 'lock', 'unLock', 'setError', 'getError'];
...
function () {
  function DataHub(config) {
    ...
    this._config = config || {};
    this.devLog = devLog;
    this.errLog = errLog; // 需要放在ConfigManager之前
    ...
    this._errorMSg = {};
    this.extendData = {};
    this.extendConfig = {}; // Emitter -> Controller -> 其它
    ...
    this.dstroyedErrorLog = (0, _Utils.createDestroyedErrorLog)('DataHub', this._key);

    this._emitter.on('$$status', function (_ref) {
      var name = _ref.name,
          value = _ref.value;

      if (value !== 'error') {
        delete _this._errorMSg[name];
      }
    });
  }
  ...
  _createClass(DataHub, [{
    ...
    value: function set(name, value) {
      ...
      if (/\_|\$/g.test(name.charAt(0))) {
        this.errLog("".concat(name, " can't be start with $ or _"));
        return;
      }
    ...
    value: function remove(name) {
      ...
      if (this._eternalData.indexOf(name) !== -1) {
        this.errLog("can't remove ".concat(name, " if it is eternal."));
        return;
      }
    ...
    key: "setError",
    value: function setError(name, msg, value) {
      if ((0, _Utils.isNvl)(name)) {
        return;
      }
    ...
    key: "setStatus",
    value: function setStatus(name, value, errMsg) {
      if ((0, _Utils.isNvl)(name)) {
        return;
      }
      ...
      if (value === 'loading') {
        this._oldStatus[name] = this.getStatus(name); // this.devLog('this._oldStatus', this._oldStatus[name], typeof this._oldStatus[name])
      }
